<template>
  <div class="founding">
    ...
    <div class="founding-ctr-wrapper" >
      <div class="founding-ctr"  >
    ...
    >
     ...
     <el-table-column align="center" label="商品" width="180">
        <template slot-scope="{ row }">
          <el-popover
    placement="top-start"
    width="200"
    trigger="hover"
    :disabled="row.goods.length<10"
    :content="row.goods">
    <div slot="reference" style="width: 100%;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">  {{ row.goods }}</div>
  </el-popover>
  ...
  methods: {
    ...
    onRefund(id) {
      this.$confirm("您确定要退款吗？", "提示", {
        ...
        goodsrefund({ id, store_id: this.id }).then((res) => {
          if (res.code == 0) {
    ...
    onExport() {
      let href =
        ...
        "/cashier/report/goods-order/export?store_id=" +
        this.id +
        "&room_type_id=" +
        this.search.type +
        "&room_id=" +
...
// .coustom-pagination-compinentss{
//     display: flex;
