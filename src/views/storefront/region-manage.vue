	...
	<div class="region-manage">
      ...
      <div></div>
		...
		<el-table :data="list" fit border highlight-current-row @row-dblclick="onEdit" :header-cell-style="{background:'#f5f7fa',color:'#606266'}" row-key="id" v-loading="loading">
			<el-table-column align="center" prop="date" label="区域代码" width="80">
			...
			<el-table-column align="center" prop="name" label="排序" width="80">
				<template slot-scope="{ row }">
					{{row.sort}}
				</template>
			</el-table-column>
			...
			<el-table-column align="center" prop="name" label="操作" width="240"  style="min-width: 240px;">
				<template slot-scope="{ row }">
          <el-popconfirm :confirm-button-text="row.status == 1? '下架':'上架'" cancel-button-text='取消' icon="el-icon-info"  :icon-color="row.status== 1 ? 'red':'green'"
            :title="row.status == 1?'下架此区域将会影响房台关联数据':'确定上架吗'" @onConfirm="onStatus(row)" >
		...
		<el-dialog :title="dialogText" :visible.sync="dialogVal" :close-on-click-modal="false" width="500px" >
			<el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="80px" @keyup.enter.native="isSave?'': onConfirm()">
				...
				<el-form-item label="排序" prop="order"><el-input :min="0" @input="temp.order = temp.order.replace(/[^0-9]/g,'')" v-model.trim="temp.order" type="number" placeholder="请输入排序" style="width:300px;"/></el-form-item>
...
<script>
  import { getToken } from '../../utils/auth.js'
	...
	import { arealist, areaadd, areainfo, areaupdate, areadelete, areastatus } from '../../api/api.js'
export default {
	name:'RegionManage',
	data() {
		return {
	...
	methods:{
		init(){
			this.loading = true;
			arealist(this.id,this.pages.page,this.pages.pageSize).then(res=>{
      ...
      this.temp.status = 1
          ...
          this.isSave = true
					if(this.choseId > 0) {
						areaupdate({
							...
							this.$message.success('编辑' + this.temp.name + '区域成功')
		...
		onDel(id) {
			this.$confirm('删除此区域会影响房台关联数据！', '提示', {
			  confirmButtonText: '确定',
			  cancelButtonText: '取消',
			  type: 'warning'
			}).then(() => {
				areadelete({id}).then(res=>{
					if(res.code == 0) {
