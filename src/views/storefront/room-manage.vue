<template>
		...
		<div class="room-manage-top">
			<div class="room-manage-top-lft">
				<div class="room-manage-top-item">
					...
					<el-input v-model="search.name" placeholder="请输入房台名称" style="width: 200px;" @keyup.enter.native="onSearch" ></el-input>
				</div>
				<div class="room-manage-top-item">
    ...
    <div style="background-color: #fff;padding: 20px 10px;box-sizing: border-box;border-radius: 10px;">
      <div class="room-manage-menu">
      	<div class="room-manage-menu-item">区域</div>
      	<div class="room-manage-menu-item" :class="isTopActive == -1?'room-manage-menu-item-active':''"  @click="ontoptAc(-1)">全部</div>
      	<div class="room-manage-menu-item" v-for="(item,index) in regionList" :key="index"  :class="isTopActive == index ?'room-manage-menu-item-active':''" @click="ontoptAc(index)">
      		<span>{{item.name}}</span>
      	</div>
      	<router-link to="./region-manage" class="room-manage-menu-item" style="text-decoration: underline;">
      ...
      <div class="room-manage-content" style="display: flex;">
      	<div class="room-manage-menurgt">
      		...
      		<div class="room-manage-menurgt-item" :class="isLftActive == -1?'room-manage-menurgt-item-active':''" @click="onlfttAc(-1)">全部</div>
      		<div class="room-manage-menurgt-item" :title="item.name" v-for="(item,idx) in typeList" :key="idx"  :class="isLftActive == idx ?'room-manage-menurgt-item-active':''" @click="onlfttAc(idx)">
              <span style="display: inline-block; width: 100%;text-overflow: ellipsis;white-space: nowrap;overflow: hidden;" >{{item.name}}</span>
      		</div>
      		<router-link to="./room-type" class="room-manage-menurgt-item" style="text-decoration: underline;">
      			<i class="el-icon-edit-outline"></i>
      			编辑
      		</router-link>
      	</div>
      	<div  style="width: calc(100% - 120px)">
      		<el-table class="table-fixed" ref="table" :data="list" fit border highlight-current-row @row-dblclick="onEdit" :header-cell-style="{background:'#f5f7fa',color:'#606266'}" v-loading="loading">
      			<el-table-column align="center" label="房台代码" width="150" fixed>
      				<template slot-scope="{ row }">
      					{{row.id}}
      				</template>
      			</el-table-column>
      			<el-table-column align="center" label="房型" style="min-width: 80px;">
      				<template slot-scope="{ row }">
      					{{row.room_type}}
      				</template>
      			</el-table-column>
      			...
      			<el-table-column align="center" label="软件版本" style="min-width: 150px;">
      				<template slot-scope="{ row }">
      					{{row.app_version}}
      				</template>
      			</el-table-column>
                ...
                <el-popconfirm :confirm-button-text="row.status == 1? '下架':'上架'" cancel-button-text='取消' icon="el-icon-info"  :icon-color="row.status== 1 ? 'red':'green'"
                  :title="row.status == 1?'确定下架吗':'确定上架吗'" @onConfirm="onStatus(row)" >
		...
		<el-dialog :title="dialogText" :visible.sync="dialogVal" :close-on-click-modal="false" width="700px">
			<el-form ref="dataForm" :inline="true" :rules="rules" :model="temp" label-position="right" label-width="100px" @keyup.enter.native="isSave?'': onConfirm()">
				<div class="room-manage-item">
					<el-form-item label="房台名称" prop="name"><el-input ref="diainput1" v-model="temp.name" /></el-form-item>
					<el-form-item label="房型" prop="type">
						<el-select v-model="temp.type" placeholder="请选择房型" style="width: 85%;">
							<el-option v-for="(item,index) in typeList" :key="item.id" :label="item.name" :value="item.id"></el-option>
						</el-select>
					</el-form-item>
				</div>
				<div class="room-manage-item">
					<el-form-item label="MAC" prop="mac"><el-input v-model.trim="temp.mac" /></el-form-item>

					<el-form-item label="区域" prop="region">
						<el-select v-model="temp.region" placeholder="请选择区域" style="width: 85%;">
							<el-option v-for="(item,index) in regionList" :key="item.id" :label="item.name" :value="item.id"></el-option>
						</el-select>
					</el-form-item>
				</div>
				<div class="room-manage-item">
					<el-form-item label="CPU"><el-input v-model.trim="temp.cpu" /></el-form-item>
					<el-form-item label="状态" prop="status">
						<el-select v-model="temp.status" placeholder="请选择状态" style="width: 85%;">
          ...
          <el-form-item>
            <el-input class="room-manage-ip" type="number" maxlength="3" oninput="value=value.replace(/[^\d.]/g,'')" v-model="temp.num1" @keyup.native.190="onInput(0)"/>
            <sub style="font-weight: bold;vertical-align:baseline;margin: 0 5px;">·</sub>
          </el-form-item>
					<el-form-item>
					  <el-input class="room-manage-ip" type="number" maxlength="3" v-model="temp.num2" oninput="value=value.replace(/[^\d.]/g,'')" ref="num2" @keyup.native.190="onInput(1)"/>
            <sub style="font-weight: bold;vertical-align:baseline;margin: 0 5px;">·</sub>
					</el-form-item>
          <el-form-item>
            <el-input class="room-manage-ip" type="number" maxlength="3" v-model="temp.num3" oninput="value=value.replace(/[^\d.]/g,'')" ref="num3" @keyup.native.190="onInput(2)"/>
            <sub style="font-weight: bold;vertical-align:baseline;margin: 0 5px;">·</sub>
          </el-form-item>
          <el-form-item>
            <el-input class="room-manage-ip" type="number" maxlength="3" v-model="temp.num4" ref="num4"/>
          </el-form-item>
				</el-form-item>
...
<script>
  ...
  import { getToken } from '../../utils/auth.js'
	import { roomlist, roomadd, roominfo, roomupdate, roomdelete, roomstatus, areaselect, roomtypeselect} from '../../api/api.js'
export default {
	name:'RoomManage',
	data() {
		return {
			...
			rules: {
				name: [{ required: true, message: '房台名称不能为空', trigger: 'blur' }],
        ...
        num1: [{ required: true, message: 'IP的第一位不能为空', trigger: 'blur' }],
        num2: [{ required: true, message: 'IP的第二位不能为空', trigger: 'blur' }],
        num3: [{ required: true, message: 'IP的第三位不能为空', trigger: 'blur' }],
        num4: [{ required: true, message: 'IP的第四位不能为空', trigger: 'blur' }],
			},
			pages:{
				page:1,
				pageSize:10,
    ...
    doLayout(e){
      ...
      this.$nextTick(() => {
		...
		initList(){
			...
			roomlist(this.id,regionId,roomId,this.search.name,this.search.status,this.pages.page,this.pages.pageSize).then(res=>{
    ...
    onSearchZero(){
      ...
      this.temp.status = 1
		...
		onConfirm(){
          ...
          this.isSave = true
          let ip = ''
          if(this.temp.num1 && this.temp.num2 && this.temp.num3 && this.temp.num4) {
            ip = this.temp.num1 + '.' + this.temp.num2 + '.' +this.temp.num3 + '.' + this.temp.num4
          }
          if(this.choseId > 0) {
            roomupdate({
              id: this.choseId,
